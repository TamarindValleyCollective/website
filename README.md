...
- **Structured data** — every page emits JSON-LD (Google's recommended schema.org format) via `src/components/JsonLd.astro`, composed in `BaseLayout.astro` with a shared `Organization`/`WebSite` graph plus per-page types (`Event`, `Service`, `Article`, `BreadcrumbList`, etc.) declared alongside each page's own content.
