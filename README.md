...
- **Site search** (magnifying-glass icon in the nav, or press `/`, `src/components/SiteSearch.astro`) — a lightweight client-side search with no backend at all. `scripts/build-search-index.mjs` runs after every `astro build` (alongside `build-chat-context.mjs`), reading each non-`noindex` page's actual rendered `<title>`/`<meta description>` out of `dist/` and writing `dist/search-index.json` — a plain static asset the search box fetches once and matches/ranks entirely in the browser. Keyboard-navigable (↑/↓/Enter, Escape to close).
...
├── scripts/build-search-index.mjs  post-build step that builds the site search's index
