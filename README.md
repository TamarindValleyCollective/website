...
- **Membership and general enquiry forms** (`/contact`, `netlify/functions/enquiry.mts`) — two plain HTML forms (name, email, optional phone, message), each submitted via [Netlify Forms](https://docs.netlify.com/manage/forms/setup/) (`data-netlify="true"` plus a honeypot field) for email — `contact@tvc.farm` needs adding as a notification recipient per form name in the Netlify dashboard, same as the Visit inquiry form below. Each form's `<script>` also fires `navigator.sendBeacon` to `/api/enquiry` alongside the native submission, appending a row to that form's own Google Sheet (`MEMBERSHIP_ENQUIRY_SHEET_ID` / `GENERAL_ENQUIRY_SHEET_ID`, shared Editor-access with the same service account photo-pool uses). The general enquiry form also offers a `wa.me` link straight to Madhavan. Both redirect to `/contact/thanks` on success.
