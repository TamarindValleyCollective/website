...
Individual **event pages** can link out to a Razorpay-hosted **Payment Link** (`rzp.io/...`) for
that event's registration fee — created and shared manually per event, not generated by the
site. Foraging Day was originally set for Aug 1 2026 with a link at
`https://rzp.io/rzp/Pry5rI8r` (fixed `66614ab`, 2026-07-17), but was postponed indefinitely and
set `draft: true` (`a424212`, 2026-07-25) — draft events are filtered out of both `/events` and
their own detail route. It's since been rescheduled to **10 Oct 2026**; the file was renamed to
match (`2026-10-10-foraging-day.md`, new URL `/events/2026-10-10-foraging-day`) and a fresh
Payment Link created — the old one had gone untracked by `fetch_all_payment_links` and the
event's price had since firmed up from an earlier ₹2,200 test link to ₹2,250, so rather than
reuse either stale link, a new one was made to match:

| Event | Amount | Link | Reference ID | Expires |
|---|---|---|---|---|
| Foraging Day (10 Oct 2026) | ₹2,250 | https://rzp.io/rzp/RjF4hjJ1 | `foraging-day-2026-10-10` | 2026-10-10 23:59 IST |

Notifications off, embedded directly on the event page's `price`/CTA (this is the first event to
use the current template's `price` + `whatsapp` frontmatter fields alongside a direct payment
link rather than the inquiry-first `BookingInquiry` flow — see `src/content/events/_template.md`
and `content.config.ts`).

Two new Payment Links were created 2026-09-06 for **3Bs&1H Edition 6** (Oct 3-4, 2026), one per
